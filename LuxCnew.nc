...
implementation
{
  ...
  event void Read.readDone(error_t error, uint16_t value)
  {
    ...
    if (error==SUCCESS && hr_counter % 6 == 0 && hr_counter <= 36) {
      lux_hi[(hr_counter/6) - 1] = ((lux >> 8) & 0xff);
      lux_low[(hr_counter/6) - 1] = ((lux >> 0) & 0xff);
      call Leds.led2Toggle();
    } else if (hr_counter > 36) {
      call Leds.led2Off();
    }
  }
